...
public class DnsDiscovery extends AbstractDiscovery
{
	...
	protected Void doInBackground(Void... params)
	{
		if (mDiscover != null)
		{
			...
			if (discover != null)
			{
				Log.i(TAG, "start=" + NetInfo.getIpFromLongUnsigned(start) + " (" + start + "), end=" + NetInfo.getIpFromLongUnsigned(end) + " (" + end + "), length=" + size);

				int timeout = Integer.parseInt(discover.prefs.getString(Prefs.KEY_TIMEOUT_DISCOVER, Prefs.DEFAULT_TIMEOUT_DISCOVER));
