...
import sarah.android.connection.Connection;
import sarah.android.connection.PersistenceException;
...
public class Persistence extends SQLiteOpenHelper implements BaseColumns
{
	...
	"CREATE TABLE " + TABLE_CONNECTIONS + " (" + _ID + " INTEGER PRIMARY KEY," + COLUMN_HOST + TEXT_TYPE + COMMA_SEP + COLUMN_client_ID + TEXT_TYPE + COMMA_SEP + COLUMN_port + INT_TYPE + COMMA_SEP + COLUMN_ssl + INT_TYPE + COMMA_SEP + COLUMN_TIME_OUT + INT_TYPE + COMMA_SEP + COLUMN_KEEP_ALIVE + INT_TYPE + COMMA_SEP + COLUMN_USER_NAME + TEXT_TYPE + COMMA_SEP + COLUMN_PASSWORD + TEXT_TYPE + COMMA_SEP + COLUMN_CLEAN_SESSION + INT_TYPE + COMMA_SEP + COLUMN_TOPIC + TEXT_TYPE + COMMA_SEP
			+ COLUMN_MESSAGE + TEXT_TYPE + COMMA_SEP + COLUMN_QOS + INT_TYPE + COMMA_SEP + COLUMN_RETAINED + " INTEGER);";
	...
	public void persistConnection(Connection connection) throws PersistenceException
	{
		...
		values.put(COLUMN_CLEAN_SESSION, conOpts.isCleanSession() ? 1 : 0); // convert
																			...
																			// in
																			// values
		values.put(COLUMN_PASSWORD, password != null ? String.valueOf(password) : null); // convert
																							// char[]
																							// to
																							// String
		values.put(COLUMN_MESSAGE, lastWill != null ? new String(lastWill.getPayload()) : null); // convert
																									// byte[]
																									// to
																									// string
		...
		{ // Successfully persisted assigning persistecneID
	...
	{
		// columns to return
		String[] connectionColumns = { COLUMN_HOST, COLUMN_port, COLUMN_client_ID, COLUMN_ssl, COLUMN_KEEP_ALIVE, COLUMN_CLEAN_SESSION, COLUMN_TIME_OUT, COLUMN_USER_NAME, COLUMN_PASSWORD, COLUMN_TOPIC, COLUMN_MESSAGE, COLUMN_RETAINED, COLUMN_QOS, _ID
		...
		{
			if (!c.moveToNext())
			{ // move to the next item throw persistence exception, if it fails
